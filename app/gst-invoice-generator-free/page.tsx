...
import {
  ArrowRight, CheckCircle2, FileText, Plus,
  ShieldCheck, Sparkles, Trash2, Loader2,
} from "lucide-react";

// ── Constants ────────────────────────────────────────────────────────────────

const AUTH_URL = "https://business.kampony.com/auth";
const FREE_INVOICE_LIMIT = 10; // Keep in sync with pricing page

// Fail loudly at startup if the API URL is missing — avoids silent localhost calls in prod.
if (typeof window === "undefined" && !process.env.NEXT_PUBLIC_API_URL) {
  console.warn(
    "[gst-invoice-generator] NEXT_PUBLIC_API_URL is not set. PDF downloads will fail in production.",
  );
}
...
const GENERATOR_FAQS = [
  {
    q: "Is the GST invoice generator really free?",
    a: "Yes. Build invoices without signing up. Sign in with Google to download the PDF — free, with a Kampony watermark. Upgrade for watermark-free PDFs.",
  },
  {
    q: "Are these invoices GST-compliant in India?",
    a: "Yes. Invoices include seller and buyer details, GSTIN, HSN/SAC codes, taxable value and CGST/SGST or IGST as required by Indian GST law.",
  },
  {
    q: "Why do I need to sign in to download?",
    a: "We use Google sign-in to create your free Kampony account so you can access your invoices later. No password needed — one click with Google.",
  },
  {
    q: "Can I generate e-way bills and IRN?",
    a: "Yes — Premium users can generate e-way bills, IRN and QR codes directly from invoices in one click.",
  },
  {
    q: "Do I need to install anything?",
    a: "No. Kampony is 100% cloud-based and works in any browser. There's also a Telegram bot for billing on the go.",
  },
  ...SHARED_FAQS,
];

const faqSchema = buildFaqSchema(GENERATOR_FAQS);
...
const inputCls =
  "w-full border border-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary bg-background";
const selectCls =
  "w-full border border-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary bg-background";
...
export default function GstInvoiceGeneratorFreePage() {
  ...
  const [seller, setSeller] = useState({
    name: "", gstin: "", address: "", city: "", state: "",
    pincode: "", phone: "", email: "", upi: "",
  });
  ...
  const totals = useMemo(() => {
    ...
    return {
      ...
      tax,
      total,
      cgst: region === "intra" ? tax / 2 : 0,
      sgst: region === "intra" ? tax / 2 : 0,
  ...
  const validate = useCallback((): string | null => {
    if (!seller.name.trim()) return "Add your business name before downloading.";
    if (items.length === 0) return "Add at least one line item.";
    const emptyItem = items.find((it) => !it.name.trim());
    if (emptyItem) return "All line items need a name.";
    const zeroItem = items.find((it) => it.qty <= 0 || it.price <= 0);
    if (zeroItem) return `"${zeroItem.name || "A line item"}" has zero quantity or price.`;
    if (!API_URL) return "Download service is not configured. Please try again later.";
    return null;
  }, [seller.name, items]);
  ...
  const handleDownload = useCallback(async () => {
    ...
    // Reset upsell so it can re-trigger on every successful download
    setShowUpsell(false);
    setDownloading(true);

    try {
      // Wait up to 5 s for the GSI SDK to become available
      const g = await new Promise<any>((resolve, reject) => {
        if ((window as any).google) { resolve((window as any).google); return; }
        let attempts = 0;
        const poll = setInterval(() => {
          if ((window as any).google) { clearInterval(poll); resolve((window as any).google); }
          else if (++attempts >= 50) {
            clearInterval(poll);
            reject(new Error("Google SDK failed to load. Please refresh."));
          }
        }, 100);
      });
      ...
      const googleToken = await new Promise<string>((resolve, reject) => {
        ...
        g.accounts.id.prompt((n: any) => {
          if (n.isNotDisplayed() || n.isSkippedMoment())
            reject(new Error("Please allow the sign-in popup"));
        });
      });
      ...
      const count = parseInt(res.headers.get("X-Invoice-Count") || "1", 10);
      ...
      a.href = url;
      a.download = "invoice-kampony.pdf";
      a.click();
  ...
  }, [validate, showToast, seller, buyer, items, region, invoiceNumber, invoiceDate]);

  // ── Render ───────────────────────────────────────────────────────────────

  const remainingFree = Math.max(FREE_INVOICE_LIMIT - invoiceCount, 0);
    ...
    <SiteLayout>
      ...
      {toast && (
        <div
          ...
          className={`fixed bottom-6 right-6 z-50 px-5 py-3 rounded-xl shadow-lg text-sm font-medium text-white ${
            toast.type === "ok" ? "bg-accent" : "bg-destructive"
          }`}
        >
          {toast.msg}
        </div>
      ...
      <section className="container py-16 md:py-24 text-center max-w-3xl">
        <div className="inline-flex items-center gap-2 rounded-full bg-primary/10 text-primary px-3 py-1 text-xs font-semibold mb-5">
          <Sparkles className="h-3.5 w-3.5" aria-hidden /> Free Tool
        </div>
        ...
        <p className="mt-5 text-lg text-muted-foreground">
          Create GST-compliant invoices in seconds. Auto CGST/SGST/IGST, HSN/SAC support and instant PDF download.
          Sign in with Google to download — free.
        </p>
      ...
      <section className="container pb-20 space-y-6">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <Field label="GSTIN">
                <input
                  className={inputCls}
                  value={seller.gstin}
                  onChange={(e) => setSeller((s) => ({ ...s, gstin: e.target.value }))}
                  placeholder="27XXXXX"
                />
              </Field>
              <Field label="Phone">
              ...
              <Field label="City">
                <input className={inputCls} value={seller.city} onChange={(e) => setSeller((s) => ({ ...s, city: e.target.value }))} />
              </Field>
              <Field label="State">
                <input className={inputCls} value={seller.state} onChange={(e) => setSeller((s) => ({ ...s, state: e.target.value }))} />
              </Field>
              <Field label="UPI ID">
                <input
                  className={inputCls}
                  value={seller.upi}
                  onChange={(e) => setSeller((s) => ({ ...s, upi: e.target.value }))}
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <Field label="GSTIN">
                <input
                  className={inputCls}
                  value={buyer.gstin}
                  onChange={(e) => setBuyer((b) => ({ ...b, gstin: e.target.value }))}
                  placeholder="27XXXXX (optional)"
                />
              </Field>
              <Field label="Phone">
                <input
              ...
              <Field label="City">
                <input className={inputCls} value={buyer.city} onChange={(e) => setBuyer((b) => ({ ...b, city: e.target.value }))} />
              </Field>
              <Field label="State">
                <input className={inputCls} value={buyer.state} onChange={(e) => setBuyer((b) => ({ ...b, state: e.target.value }))} />
              </Field>
            </div>
          </div>
        </div>
        ...
        <div className="grid lg:grid-cols-5 gap-6">
          <div className="glass-card rounded-2xl p-6 space-y-4 lg:col-span-3">
            ...
            {items.map((it) => (
              <div key={it.id} className="grid grid-cols-12 gap-2 items-center">
                ...
                <select
                  aria-label="GST rate"
                  className={`${selectCls} col-span-9 md:col-span-2`}
                  value={it.rate}
                  onChange={(e) => updateItem(it.id, { rate: parseFloat(e.target.value) })}
                >
                ...
                <button
                  ...
                  className="col-span-3 md:col-span-1 flex justify-center text-muted-foreground hover:text-destructive transition-colors"
                >
            ...
            <button
              ...
              className="flex items-center gap-1.5 text-sm border border-border rounded-lg px-3 py-2 hover:bg-secondary transition-colors"
            >
          ...
          <div className="border border-border rounded-2xl p-6 lg:col-span-2">
                  ...
                  <div key={it.id} className="flex justify-between text-sm">
                    <div className="min-w-0 pr-3">
                      ...
                      <div className="text-xs text-muted-foreground">
                        {it.hsn && `HSN ${it.hsn} · `}{it.qty} × {inr(it.price)} · {it.rate}%
                      </div>
                    </div>
            ...
            <div className="border-t border-border mt-5 pt-4 space-y-2 text-sm" aria-live="polite">
            ...
            <button
              ...
              className="w-full mt-6 flex items-center justify-center gap-2 bg-primary text-primary-foreground px-4 py-3 rounded-xl font-semibold hover:opacity-90 disabled:opacity-60 transition-opacity"
            >
              {downloading
                ? <><Loader2 className="h-4 w-4 animate-spin" /> Generating...</>
                : <>Download PDF — Free <ArrowRight className="h-4 w-4" /></>}
            </button>
            <p className="text-[11px] text-center text-muted-foreground mt-2">
              Sign in with Google to download · Kampony watermark on free PDFs
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="container py-16 md:py-24 max-w-3xl">
        ...
        <div className="mt-6 space-y-5 text-muted-foreground leading-relaxed">
          <p>
            A GST invoice is a legal document issued by a registered seller to the buyer, showing the goods or
            services supplied along with the GST charged. It is required under Indian GST law for B2B and many
            B2C transactions.
          </p>
        ...
        >
          <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full overflow-hidden">
            ...
            <div className="bg-gradient-hero p-6 text-white text-center relative">
              ...
              <h2 id="upsell-title" className="text-xl font-bold">Invoice Downloaded!</h2>
              <p className="text-white/80 text-sm mt-1">
                {remainingFree === 0
                  ? `You've used all ${FREE_INVOICE_LIMIT} free invoices`
                  : `${remainingFree} free invoice${remainingFree !== 1 ? "s" : ""} remaining`}
              </p>
            </div>
            ...
            <div className="p-6">
              ...
              <ul className="space-y-2.5 mb-6">
                {[
                  ...
                  { icon: "🎨", text: "6 professional invoice templates" },
                  ...
                  <li key={feat.text} className="flex items-start gap-2.5 text-sm">
                    <span className="text-base leading-5" aria-hidden>{feat.icon}</span>
                    <span>{feat.text}</span>
                  </li>
                ))}
              </ul>
